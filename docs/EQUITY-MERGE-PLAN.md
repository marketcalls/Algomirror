...
| 2.8 | Resolve the stuck-state set | MOSTLY DONE | PARTIAL-for-ever fixed (`is_open` now requires a live split). EXIT_PENDING-on-crash fixed (`recover_stale_exit_claims`, swept by the exit monitor, recovers to EXIT_INDETERMINATE and never to ACTIVE). INDETERMINATE-with-no-adoption-candidate still open |
...
| 4.3 | PARTIAL DONE. `equityReadJson` in equity_common.js distinguishes a refusal AlgoMirror decided itself (429, 403, 400, 422) from a genuinely lost answer, and the order submit path uses it. The remaining read endpoints still assume a JSON body. Original note: check `response.ok` on every write. Module-wide it is checked zero times, so a 429 or 403 currently reads as "may have reached the broker" when nothing was sent. Our baseline shares this gap; his commit multiplies the endpoints it applies to |
...
| 4.5 | NOT NEEDED in our tree. Our Place Order already shows Product, fixed to CNC, which is right for a delivery-only module (PRD 2.1). It is Saravanan's M4 that removed the control; do not inherit that when adopting his template |
| 4.6 | PARTIAL DONE. Est. Costs and Net Value now appear per fill and in the totals on the Trade Book (PRD 7.8), which only became meaningful once fills were real. Holdings already had them. Place Order > Order Status still does not |
